...
Será apresentado neste capítulo as bases teóricas dos principais modelos analíticos utilizados na realização deste trabalho, de forma a esclarecer dúvidas quanto a sua utilização no decorrer dos próximos capítulos do presente estudo. Tais métodos serão incluídos na metodologia proposta pelo autor, concebida de tal forma que o desenvolvimento deste trabalho estivesse alinhado com o objetivo inicialmente definido. Também será feita a apresentação do objeto de estudo e de todas as entidades envolvidas e interessadas nele.
...
\item Condensação de dados de diversas fontes em um mesmo modelo
\end{enumerate} 
...
Após a realização de uma pesquisa com um alto número de pessoas, um dos principais obstáculos do pesquisador é analisar um grande volume de dados em tempo hábil e eficaz em relação à absorção de informação do conteúdo apresentado. Em muitos casos, a falta de conhecimento de métodos de análise leva os analistas a lerem e relerem individualmente todas as respostas de forma a terem uma interpretação mais profunda dos dados, porém a eficiência é muito menor, não significando uma interpretação melhor dos resultados. De forma a sintetizar as informações presentes em mensagens (escritas, orais ou qualquer outro meio de comunicação), a análise de conteúdo permite unir a camada lógica da linguistica com a semântica da hermenêutica para interpretar essas mensagens.

A análise de conteúdo é um dos métodos mais utilizados para a avaliação de estudos qualitativos e consiste em um conjunto de técnicas de análise de comunicações que permitem encontrar os principais significados de um grande volume de mensagens, através de métodos lógicos e semânticos. Ela tem como principal técnica a inferência, que consiste em produzir suposições subliminares sobre determinadas mensagens, embasando-se no contexto em que a mensagem é produzida e recebida. \cite{bardin}

Um elemento interessante dessa metodologia é a sua capacidade de gerar análises quantitativas e qualitativas, e até modelos híbridos quando for de interesse da pesquisa. Como ela trata principalmente do sentido dos elementos presentes no conteúdo das mensagens, ela pode sugerir tanto uma contagem de frases e palavras e obter inferências acima das de maior ocorrência quanto sugerir uma análise subjetiva do contexto em que essa comunicação foi feita, como o estado emocional dos comunicadores.

Entre as diversas aplicações demonstradas por \citeonline{bardin}, é possível observar 4 etapas recorrentes presentes na utilização da Análise de Conteúdo. 
...
\caption* {Fonte: Elaborado a partir dos exemplos empíricos de \citeonline{bardin}}
\end{figure}
...
Segundo \citeonline{empreendedorismo}, o empreendedorismo atrai alunos com as seguintes características:
